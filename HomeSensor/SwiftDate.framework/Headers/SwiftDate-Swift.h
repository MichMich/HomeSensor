...
+ (NSCalendar * __null_unspecified)locale:(BOOL)autoUpdate;
@end
...
/// Initialize a new NSDate objects from passed components object. Returned object is expressed as absolute time in UTC timezone. Where there are insufficient components provided to completely specify an absolute time, a calendar uses values of its choice. If inconsistences were found formatter may ignore problematic components or return nil object.
///
/// \param components components. If your time is expressed in another format remeber to specify it in components.timeZone
...
- (nullable instancetype)initWithComponents:(NSDateComponents * __nonnull)components;
...
- (NSDate * __nonnull)add:(NSDateComponents * __nonnull)components;

/// The same of calling components() without specify a region: UTC region is used instead
@property (nonatomic, readonly, strong) NSDateComponents * __nonnull components;
@end
...
@property (nonatomic, readonly, strong) NSDate * __nonnull fromNow;

/// The same of calling ago() with default UTC region
@property (nonatomic, readonly, strong) NSDate * __nonnull ago;
@end
